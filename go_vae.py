...
class goVAE(nn.Module):
	...
	def __init__(self, save_dir='', lr=1e-3, z_dim=32, model_precision=10.0, beta=2, \
	            use_old=1,use_new=1, zhat=True, old_latent=[], device_name="auto"):
		"""Construct a VAE.
	...
	def train_epoch(self, train_loader, old_data, z_o):
		...
		for batch_idx, data in enumerate(train_loader):
			...
			loss, rec, ent = self.forward(data, return_latent_rec=False)
			old_loss = self.Beta * self.old_data_loss(old_data,z_o)
