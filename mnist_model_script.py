...
def mnist_model_script(n_train_runs = 10, root = '',figs=False):
    ...
    dsb = np.vstack([ds1,ds2])
    labelsb = np.hstack([labels1,labels2])
    ...
    train_dataset_all = qpvae_ds((list(dsb),list(labelsb)), transform = torch.tensor)


    ##### Sequential template gets ONE of these
    train_dataset1 = qpvae_ds((list(ds1), list(labels1)), transform = torch.tensor)
    train_dataset2 = qpvae_ds((list(ds2), list(labels2)), transform = torch.tensor)
    ...
    for qp_ind, n_qp in enumerate(n_qps):
        ...
        train_dataset2j_copy.x = np.vstack((train_dataset2j_copy.x,oldx1j))
        train_dataset2j_copy.y = np.hstack([train_dataset2j_copy.y,oldy1j])
        train_dataset2s_copy.x = np.vstack((train_dataset2s_copy.x,oldx1s))
    ...
    for run_ind in range(n_train_runs):
        ...
        for qp_ind, n_qp in enumerate(n_qps):
            ...
            vae_1j_anchor_save = os.path.join(root,'vae1j_mnist_run_'+str(run_ind) + '_nqp_' + str(n_qp))
            vae_1s_anchor_save = os.path.join(root,'vae1s_mnist_run_'+str(run_ind) + '_nqp_' + str(n_qp))
            bvae_1j_anchor_save = os.path.join(root,'bvae1j_mnist_run_'+str(run_ind) + '_nqp_' + str(n_qp))
            bvae_1s_anchor_save = os.path.join(root,'bvae1s_mnist_run_'+str(run_ind) + '_nqp_' + str(n_qp))
            qpvae_1j_anchor_save = os.path.join(root,'qpvae1j_mnist_run_'+str(run_ind) + '_nqp_' + str(n_qp))
            qpvae_1s_anchor_save = os.path.join(root,'qpvae1s_mnist_run_'+str(run_ind) + '_nqp_' + str(n_qp))
