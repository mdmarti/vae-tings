...
class B_VAE(nn.Module):
	...
	def forward(self, x, return_latent_rec=False):
		...
		try:
			...
			mu,u,d = self.encode_bugcheck(x)
			latent_dist = LowRankMultivariateNormal(mu, u, d)
		#print(u.shape)
		v = torch.matmul(u,u.permute(0,2,1)) + torch.diag_embed(d)
		mu.unsqueeze_(2)
		#print(mu.shape)
		#print(v.shape)
		...
		elbo = pxz_term#elbo + pxz_term
		# D_kl[q(z|x)||p(z)]
		trace = torch.diagonal(v, dim1 = -2, dim2 = -1).sum(-1)
		means = torch.matmul(mu.permute(0,2,1),mu).squeeze()
		#print(means)
		#print(v)
		det = torch.logdet(v)
		#print(det)

		dkl =1/2 *(trace + torch.matmul(mu.permute(0,2,1),mu).squeeze() - self.z_dim  - torch.logdet(v))
		#print(dkl)

		elbo = elbo - self.beta*torch.sum(dkl)
		if return_latent_rec:
			return -elbo, z.detach().cpu().numpy(), \
				x_rec.view(-1, X_SHAPE[0], X_SHAPE[1]).detach().cpu().numpy(), -pxz_term
		return -elbo, -pxz_term, self.beta*torch.sum(dkl)
	...
	def train_epoch(self, train_loader):
		...
		print('Epoch: {} Average Rec Error: {:.4f}'.format(self.epoch, \
				train_rec))
		print('Epoch: {} Average D_kl: {:.4f}'.format(self.epoch, \
				train_ent))
		self.epoch += 1
		return train_loss
